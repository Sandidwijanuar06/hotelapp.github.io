<?php
  ...
  $id_kamar = $_GET['id'];
  $query = mysqli_query($con,"SELECT * FROM kamar WHERE kamar.id_kamar='$id_kamar'") or die(mysqli_error($con));
...
<head>
  ...
  <title>Resepsionis Page | Fasilitas Kamar</title>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Daftar Fasilitas Kamar</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <a href="../tambah/tambah_fas_kamar.php?id=<?=$data['id_kamar'];?>" class="btn btn-primary float-right"><i class="fas fa-plus"> Tambah Fasilitas</i></a>
                <h3 class="card-title">Tabel Daftar Fasilitas Kamar</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <th>Kualitas Fasilitas</th>
                    <th>Aksi</th>
                  </tr>
                  </thead>
                  <tbody>
                    <?php
                      $query = mysqli_query($con,"SELECT fasilitas_kamar.id_fas_kamar, fasilitas_kamar.id_kamar, fasilitas_kamar.nama_fasilitas, fasilitas_kamar.kualitas_fas_kamar 
                      FROM fasilitas_kamar INNER JOIN kamar WHERE fasilitas_kamar.id_kamar=kamar.id_kamar AND fasilitas_kamar.id_kamar='$id_kamar'");
                      $no = 1;
                      while($data = mysqli_fetch_array($query)){
                        $id_kamar            = $data['id_kamar'];
                        $nama_fasilitas      = $data['nama_fasilitas'];
                        $kualitas_fas_kamar  = $data['kualitas_fas_kamar'];
                        $id_fas_kamar        = $data['id_fas_kamar'];
                    ?>
                    <tr>
                      <td><?=$no++;?></td>
                      <td><?=$nama_fasilitas;?></td>
                      <td><?=$kualitas_fas_kamar;?></td>
                      <td>
                        <a href="../edit/edit_fas_kamar.php?id=<?=$data['id_fas_kamar'];?>" class="btn btn-success"><i class="fas fa-edit"></i></a>
                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#del<?=$id_fas_kamar;?>">
                        <i class="fas fa-trash"></i>
                        </button>
                      </td>
                      <!-- Modal Delete -->
                      <div class="modal fade" id="del<?=$id_fas_kamar;?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-sm">
                          <form method="post" action="?id=<?=$_GET['id'];?>">
                            ...
                            <div class="modal-body">
                              ...
                              <input type="hidden" name="id_fas_kamar" value="<?=$id_fas_kamar;?>">
                              Apakah anda yakin ingin menghapus fasilitas ini(<strong><?=$nama_fasilitas;?></strong>)? KLIK "<strong>HAPUS</strong>" jika yakin!
                            </div>

                            <div class="modal-footer">
                              <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-left"></i>Batal</button>
                              <button type="submit" class="btn btn-success" name="deletefas">Hapus</button>
                            </div>
                            </div>
                          </form>
                        </div>
