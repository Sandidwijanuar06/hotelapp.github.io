...
<head>
  ...
  <title>Resepsionis Page | Fasilitas Hotel</title>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Daftar Fasilitas Hotel</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <a href="tambah/tambah_fas_hotel.php" class="btn btn-primary float-right"><i class="fas fa-plus">Tambah Fasilitas</i></a>
                <h3 class="card-title">Tabel Daftar Fasilitas Hotel</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                      $query = mysqli_query($con,"SELECT * FROM fasilitas_hotel");
                      $no = 1;
                      while($data = mysqli_fetch_array($query)){
                        $id_fas_hotel       = $data['id_fas_hotel'];
                        $nama_fasilitas     = $data['nama_fasilitas'];
                        $kualitas_fas_hotel = $data['kualitas_fas_hotel'];
                        $foto_fas_hotel     = $data['foto_fas_hotel'];
                    ?>
                    <tr>
                      <td><?=$no++;?></td>
                      <td><?=$nama_fasilitas;?></td>
                      <td><?=$kualitas_fas_hotel;?></td>
                      <td>
                        <a href="detail/detail_fas_hotel.php?id=<?=$data['id_fas_hotel'];?>" class="btn btn-warning"><i class="fas fa-eye"></i></a>
                        <a href="edit/edit_fas_hotel.php?id=<?=$data['id_fas_hotel'];?>" class="btn btn-success"><i class="fas fa-edit"></i></a>
                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#del<?=$id_fas_hotel;?>">
                        <i class="fas fa-trash"></i>
                        </button>
                      </td>
                    </tr>
                    <!-- Modal Delete -->
                    <div class="modal fade" id="del<?=$id_fas_hotel;?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-sm">
                        <form method="get">
                          <div class="modal-content">
                          <div class="modal-header">
                              <h5 class="modal-title" id="exampleModalLabel">Delete Fasilitas Hotel</h5>
                          ...
                          <div class="modal-body">
                            ...
                            <input type="hidden" name="foto_fas_hotel" value="<?=$foto_fas_hotel;?>">
